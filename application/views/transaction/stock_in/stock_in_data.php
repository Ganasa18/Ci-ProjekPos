  ...
  <div class="container-fluid">
    <div class="row">
      <div class="card col-lg">
        ...
        <div class="card-body">
          <div class="mb-3">
            <a href="<?= site_url('stock/stock_in_add'); ?>" class=" float-right btn btn-primary btn-flat just mb-3"><i class="fa fa-plus"></i> Tambah Stock </a>
          </div>
                ...
                <?php foreach ($stock as $key => $data) : ?>
                  <tr>
                    ...
                    <td width="250px" class="text-center">
                      <a class="btn btn-default" id="set_dtl" data-toggle="modal" data-target="#modal-detail" data-barcode="<?= $data->barcode; ?>" 
                      data-item_nama="<?= $data->item_name; ?>" data-supplier_nama="<?= $data->supplier_name; ?>" data-qty="<?= $data->qty; ?>" data-detail="<?= $data->detail; ?>" 
                      data-date="<?= indo_date($data->date); ?>">
                        <i class="fas fa-eye"></i> Detail </a>
                      <a href="<?= site_url('stock/stock_in_del/' . $data->stock_id . '/' . $data->item_id) ?>" onclick="return confirm('Apakah Anda Yakin')" class="btn btn-danger ">
...
<script>
  $(document).ready(function() {
    $(document).on('click', '#set_dtl', function() {
      ...
      var supplier_name = $(this).data('supplier_nama');
      var qty = $(this).data('qty');
      var detail = $(this).data('detail');
      var date = $(this).data('date');
      $('#barcode').text(barcode);
      $('#item_nama').text(item_name);
      $('#supplier_nama').text(supplier_name);
      $('#qty').text(qty);
      $('#detail').text(detail);
      $('#date').text(date);
